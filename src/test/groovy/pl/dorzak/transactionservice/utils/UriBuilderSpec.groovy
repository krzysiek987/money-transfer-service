...
class UriBuilderSpec extends Specification {
    ...
    def createUriBuilder(final String scheme, final String host, final Integer port, final List<String> pathSegments, final Map<String, String> queryParams) {
        def uriBuilder = new UriBuilder(scheme, host, port)
        if (pathSegments != null) {
            pathSegments.each { uriBuilder.pathSegment it }
        }
        if (queryParams != null) {
            queryParams.entrySet().each { uriBuilder.queryParam(it.getKey(), it.getValue()) }
        }
        return uriBuilder
    }
